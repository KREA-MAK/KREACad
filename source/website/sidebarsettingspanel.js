...
class SettingsModelDisplaySection extends SettingsSection {
  constructor(parentDiv, settings) {
    ...
    this.gradientToggle = null;
    this.gradientTopColorPicker = null;
    this.gradientBottomColorPicker = null;
    this.gradientColorsDiv = null;
  ...
  Init(callbacks) {
    ...
    let gradientToggleDiv = AddDiv(this.contentDiv, "ov_sidebar_parameter");
    this.gradientToggle = AddToggle(
      gradientToggleDiv,
      "ov_sidebar_parameter_toggle"
    );
    AddDiv(
      gradientToggleDiv,
      "ov_sidebar_parameter_text",
      Loc("Gradient Background")
    );

    this.gradientToggle.OnChange(() => {
      this.settings.backgroundIsGradient = this.gradientToggle.GetStatus();
      ShowDomElement(this.gradientColorsDiv, this.gradientToggle.GetStatus());
      this.callbacks.onBackgroundGradientChanged();
    });

    // Gradient colors
    this.gradientColorsDiv = AddDiv(
      this.contentDiv,
      "ov_sidebar_settings_padded"
    );

    let topColorDiv = AddDiv(this.gradientColorsDiv, "ov_sidebar_parameter");
    let topColorInput = AddDiv(topColorDiv, "ov_color_picker");
    AddDiv(topColorDiv, null, Loc("Top Color"));
    let predefinedGradientColors = [
      "#e6e6faff",
      "#ffffffff",
      "#f0f8ffff",
      "#1e1e28ff",
    ...
    let defaultTopColor =
      "#" + RGBColorToHexString(this.settings.backgroundGradientTopColor);
    this.gradientTopColorPicker = AddColorPicker(
      topColorInput,
      false,
      defaultTopColor,
      predefinedGradientColors,
      (r, g, b) => {
        this.settings.backgroundGradientTopColor = new RGBColor(r, g, b);
        this.callbacks.onBackgroundGradientChanged();
      }
    );

    let bottomColorDiv = AddDiv(this.gradientColorsDiv, "ov_sidebar_parameter");
    let bottomColorInput = AddDiv(bottomColorDiv, "ov_color_picker");
    AddDiv(bottomColorDiv, null, Loc("Bottom Color"));
    let defaultBottomColor =
      "#" + RGBColorToHexString(this.settings.backgroundGradientBottomColor);
    this.gradientBottomColorPicker = AddColorPicker(
      bottomColorInput,
      false,
      defaultBottomColor,
      predefinedGradientColors,
      (r, g, b) => {
        this.settings.backgroundGradientBottomColor = new RGBColor(r, g, b);
        this.callbacks.onBackgroundGradientChanged();
      }
    );

    this.gradientToggle.SetStatus(this.settings.backgroundIsGradient);
    ShowDomElement(this.gradientColorsDiv, this.settings.backgroundIsGradient);
  ...
  Update() {
    ...
    if (this.gradientToggle !== null) {
      this.gradientToggle.SetStatus(this.settings.backgroundIsGradient);
      ShowDomElement(
        this.gradientColorsDiv,
        this.settings.backgroundIsGradient
      );
    }

    if (this.gradientTopColorPicker !== null) {
      this.gradientTopColorPicker.setColor(
        "#" + RGBColorToHexString(this.settings.backgroundGradientTopColor)
      );
    }

    if (this.gradientBottomColorPicker !== null) {
      this.gradientBottomColorPicker.setColor(
        "#" + RGBColorToHexString(this.settings.backgroundGradientBottomColor)
      );
    }
  ...
  Clear() {
    ...
    if (this.gradientTopColorPicker !== null) {
      this.gradientTopColorPicker.hide();
    }

    if (this.gradientBottomColorPicker !== null) {
      this.gradientBottomColorPicker.hide();
    }
